  ...
  execute(): null {
    ...
    for (const output of this.node.outputs) {
      ...
      if (this.tcb.env.config.checkTypeOfOutputEvents && output.name.endsWith('Change')) {
        ...
        checkIsSplitTwoWayBinding(inputName, output, this.node.inputs, this.tcb);
      }
...
class TcbUnclaimedOutputsOp extends TcbOp {
  ...
  execute(): null {
    ...
    for (const output of this.element.outputs) {
      ...
      if (this.tcb.env.config.checkTypeOfOutputEvents && output.name.endsWith('Change')) {
        const inputName = output.name.slice(0, -6);
        if (checkIsSplitTwoWayBinding(inputName, output, this.element.inputs, this.tcb)) {
...
function checkIsSplitTwoWayBinding(
  ...
  if (outputConsumer instanceof TmplAstElement) {
    tcb.oobRecorder.splitTwoWayBinding(
        tcb.id, input, output, inputConsumer.ref.node.name, outputConsumer);
    return true;
  } else if (outputConsumer.ref !== inputConsumer.ref) {
    tcb.oobRecorder.splitTwoWayBinding(
        tcb.id, input, output, inputConsumer.ref.node.name, outputConsumer.ref.node.name);
    return true;
  }
  return false;
}
