...
export interface TemplateCheck<T extends ErrorCode> {
  ...
  run(component: ts.ClassDeclaration, template: TmplAstNode[]): NgTemplateDiagnostic<T>[];
}
...
/**
 * Function to construct a TemplateCheck with a TempalteContext;
 */
export type TemplateCheckFactory = (ctx: TemplateContext) => TemplateCheck<ErrorCode>;
...
export abstract class TemplateCheckWithVisitor<T extends ErrorCode> implements TemplateCheck<T> {
   ...
   */
  run(component: ts.ClassDeclaration, template: TmplAstNode[]): NgTemplateDiagnostic<T>[] {
    const visitor = new CompleteVisitor<T>(this.ctx, this, component)
...
class CompleteVisitor<T extends ErrorCode> extends RecursiveAstVisitor implements
  ...
  constructor(readonly ctx: TemplateContext, readonly check: TemplateCheckWithVisitor<T>, readonly component: ts.ClassDeclaration) {
    super();
  }
