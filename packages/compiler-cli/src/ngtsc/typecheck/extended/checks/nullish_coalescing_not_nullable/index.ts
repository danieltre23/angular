...
import {ExpressionSymbol, NgTemplateDiagnostic} from '../../../api';
...
export class NullishCoalescingNotNullableCheck extends
  ...
  override visitNode(ctx: TemplateContext, component: ts.ClassDeclaration, node: TmplAstNode|AST):
    ...
    const diagnostics: NgTemplateDiagnostic<ErrorCode.NULLISH_COALESCING_NOT_NULLABLE>[] = [];
    if (node instanceof Binary) {
      if (node.operation == '??') {
        // Get the type of the value on the left side.
        const symbol =
            ctx.templateTypeChecker.getSymbolOfNode((node.left as PropertyRead), component);
        ...
        if (type.getNonNullableType() === type) {
          diagnostics.push(ctx.templateTypeChecker.makeTemplateDiagnostic(
              component, span, ts.DiagnosticCategory.Warning,
