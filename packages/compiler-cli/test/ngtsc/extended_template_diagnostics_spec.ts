...
runInEachFileSystem(() => {
  describe('ngtsc extended template checks', () => {
    ...
    beforeEach(() => {
      ...
      env.tsconfig({extendedTemplateDiagnostics: true, strictTemplates: true});
    });
    ...
    it('should not produce extended diagnostics if flag is disabled', () => {
      env.tsconfig({extendedTemplateDiagnostics: false});
      env.write('test.ts', `
              import {Component} from '@angular/core';
              @Component({
                selector: 'test',
    ...
    it('should throw error if extendedTemplateDiagnostics are enabled and strictTemplates disabled',
       () => {
         env.tsconfig({extendedTemplateDiagnostics: true, strictTemplates: false});
         env.write('test.ts', `
              import {Component} from '@angular/core';
              @Component({
                selector: 'test',
         ...
         expect(diags[0].messageText)
             .toMatch(
                 /Error: The 'extendedTemplateDiagnostics' option requires 'strictTemplates' to also be enabled./);
       });
  });
});
